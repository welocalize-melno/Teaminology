		...
		getFileInfoByJobIds: function(multiJobValues,callback) {
		 ...
		 getJobsCount : function(callback) {
			...
			getUserRoleList : function(callback) {
				...
				getUserConfiguredFields : function(callback) {
					 var params = $j.extend({
		                type: "GET",
		                 url: $j("#contextRootPath").val()+"/teaminology_ctrler/teaminology/getPrevilegesByUser?random=" + Math.floor(Math.random() * (new Date()).getTime() + 1),
		                dataType: "json"
					 }, callback);
		        
					 $j.ajax(params);
				 },
				saveConfiguredFields : function(fileIds,callback) {
					var params = $j.extend({
						type: "POST",
						data : {selectedFields:fileIds},
						url: $j("#contextRootPath").val()+"/teaminology_ctrler/teaminology/saveConfiguredFields?random=" + Math.floor(Math.random() * (new Date()).getTime() + 1),
						dataType: "json"
					}, callback);
					$j.ajax(params);
				}
