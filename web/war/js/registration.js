    ...
    $j("#cancelBtn").click(function () {
        if ($j("#hpsite").val() == "true")
            window.location = "index.jsp";
        	 ...
        	 window.location = "index.jsp";
            //
        	window.location = pageUrl + "/index.jsp";
        $j(".userExistsErr").hide();
        $j(".emailExistsErr").hide();
...
//			
        ...
        $j("#uploadUserRegPic").click(function (e) {
            if ($j('#uploadRegPic').val() == "") {
                ...
                // validation to check image file
                var ext = '';
                if (fileName != null && fileName.lastIndexOf(".") > 0){
                    ext = fileName.substr(fileName.lastIndexOf(".") + 1);
	                var fileAcceptedTypes = ["jpg","jpeg","png","gif"];
	                if (ext != '' &&  $.inArray( ext.toLowerCase(), fileAcceptedTypes ) <= -1 ){
	                	 $j(".picError").html("Please upload jpeg or gif or png image file");
                         $j(".picError").show();
                         $j("#uploadPic").val("");
	                	return false;
	                }
	            }
                // completed validation for uploaded file
                ajaxFunction();
        ...
        var processStateChange = function (uploadData) {
            if (uploadData == null) {
                ...
                if(fileName == null){
                	 $j(".picError").html("Please upload image jpeg or gif or png image file");
                     $j(".picError").show();
                     $j("#uploadPic").val("");
                     return false;
                }
