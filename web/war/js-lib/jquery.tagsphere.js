...
Usage:
 ...
 Centered in a 200 x 200 container:
    $j('.tags').tagcloud({centrex:100,centrey:100});

 With a different update speed
    $j('.selector').tagcloud({fps:24});
...
(function($j){
     ...
     var TagCloudClass = function(el, options){
         ...
         var dirty = true;
         var container = $j(el);
         var id_stub = 'tc_' + $j(el).attr('id') + "_";
         var opts = options;
         var zoom = opts.zoom;
         var depth;
	 var points_meta = [];
         ...
         if (options.background_colour){
             ...
             bg_colour = parsecolour($j(el).css('background-color'));
         }
         
         function parsecolour(colour){
             function parse_rgb_colour(colour){
                 rgb = colour.match(/^rgb\((\d+),\s*(\d+),\s*(\d+)\)$j/);
         ...
         //     }
         // }
         $j('li a', container).each(function(idx, val){

                                       var sz = parseInt($j(this).attr('rel'));
                                       if(sz == 0) 
                                           sz = 1;
                                       ...
                                       points_data[idx].colour = parsecolour($j(this).css('color'));
                                       ...
                                       $j(this).addClass('point');
                                       $j(this).attr('id', point_id);

                                       //container.append('<div id="'+ point_id +'" class="point" style="position:absolute;display:none;"><a href=' + $j(this).attr('href')  + '>' + $j(this).html()  + '</a></div>');
                                       points_data[idx].element = $j('#'+point_id);
